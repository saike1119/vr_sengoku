...
public class FollowCamera : MonoBehaviour {
	...
	void LateUpdate () {
		...
		if (inputManager.Moved()) {
			...
			Vector2 delta = inputManager.GetDeltaPosition();//カーソルの移動量を求める
		...
		if (lookTarget != null) {
			...
			// 注視対象からの相対位置を求める.
			Vector3 relativePos = Quaternion.Euler(verticalAngle,horizontalAngle,0) *  new Vector3(0,0,-distance);//水平回転角度と上下回転角度とカメラと注視点間の距離、この三つが分かればこのように求めることができる。
